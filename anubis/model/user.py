...
@argmethod.wrap
async def set_volunteer(uid: int):
    return await set_priv(uid, builtin.DEFAULT_PRIV | builtin.PERM_SEND_CONTEST_BALLOON)
...
async def get_prefix_list(prefix: str, fields=PROJECTION_VIEW, limit: int=50):
    ...
    coll = db.Collection('user')
    udocs = await (coll.find({'uname_lower': {'$regex': regex}}, projection=fields).to_list(limit))
