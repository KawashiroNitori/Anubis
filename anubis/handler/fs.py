...
class FsGetHandler(base.Handler):
    ...
    async def stream_data(self, *, secret: str, headers_only: bool=False):
        ...
        self.response.headers['Cache-Control'] = 'max-age=2592000'  # 30 days
...
class FsUploadHandler(base.Handler):
    ...
    async def post(self):
        ...
        usage = await userfile.get_usage(self.user['_id'])
        if usage >= quota:
            raise error.UsageExceedError(builtin.DOMAIN_ID_SYSTEM, self.user['_id'], 'usage_userfile', usage, quota)
        ...
        self.render('fs_upload.html', fdoc=fdoc, ufid=ufid, usage=usage, quota=quota)
